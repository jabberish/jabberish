...
module.exports = Router()
  .post('/', ensureAuth, (req, res, next) => {
    ...
    Workspace.create({ name, owner: req.user._id })
      .then(workspace => {
      ...
      .then(rel => {
        return Promise.all([rel, Channel.create({ name: 'General', workspace: rel.workspace })]);
      })
  ...
  .post('/add-user/:id', ensureAuth, (req, res, next) => {
    ...
    Workspace.findById(req.params.id)
      ...
      .then(user => {
        return UserByWorkspace.create({ user: user._id, workspace: req.params.id });
      })
      .then(rel => {
        res.send(rel);
      })
  ...
  .get('/active', ensureAuth, (req, res, next) => {
    Message.getTopActiveWorkspaces()
      .then(workspaces => res.send(workspaces))
      .catch(next);
  })
