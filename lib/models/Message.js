...
messageSchema.statics.getTopChannelsByUniqueUsers = function() {
  return this.aggregate([
    { $group: { _id: '$channel', users: { $addToSet: '$user' } } },
    { $unwind: { path: '$users' } },
    ...
    { $lookup: { from: 'channels', localField: '_id', foreignField: '_id', as: 'channel' } },
    { $unwind: { path: '$channel' } },
    { $project: { users: '$count', name: '$channel.name', workspace: '$channel.workspace' } }
  ]);
};
