...
io.use(function(socket, next) {
  console.log('user attempting to connect to socket');
  ...
  socket.on('join', function(data) {
    UserByWorkspace.findOne({ user: data.user, workspace: data.workspace })
      .then(rel => {
        if (!rel._id) throw new Error('You must be a member of the workspace to join this channel');
        return Message.find({ channel: data.channel })
          .populate('user')
          .sort({ date: -1 });
      })
      .then(messages => {
